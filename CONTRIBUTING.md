...
-   If the section titles in the `docx` version of the protocol comply with section titles in the templates used by the `protocolhelper` package, than you will normally not see Rmarkdown file names starting with the same number. Otherwise, the function will have written both empty template Rmarkdown files as well as Rmarkdown files resulting from conversion of the `docx` file (Note that `NEWS.md` and `index.Rmd` are always created from the `protocolhelper` templates). In that case, you need to make changes ( renaming of files, deleting of redundant files) so that Rmarkdown file names comply with the [template Rmarkdown files](https://github.com/inbo/protocolhelper/tree/master/inst/rmarkdown/templates)
...
### From an existing docx protocol {#from-an-existing-docx-protocol}
...
### From a new template {#from-a-new-template}
