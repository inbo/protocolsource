...
Tabel \@ref(tab:checklist_boot) geeft de benodigde apparatuur en materiaal voor bootwerk in diepe wateren.
...
df <- data.frame(
  Item = c(
    ...
    "Flora van België (Verloove & Van Rossum, 2023) en/of andere determinatiewerken (zie [Bijlage 1](#bijlage1))"
  ),
...
pander(
  df,
  caption = add_label("Checklist van benodigde apparatuur en materiaal.")
)
```
```{r checklist_boot}
df <- data.frame(
...
pander(
  df,
  caption = add_label("Checklist van benodigde apparatuur en materiaal voor bootwerk in diepe wateren.")
)
```
...
knitr::include_graphics(
  file.path(path_to_protocol, "media/Fig1Hark.png")
)
```

```{r Figuur2, fig.cap = "Hark met volièredraad (foto Jo Packet)"}
knitr::include_graphics(
  file.path(path_to_protocol, "media/Fig2Hark2.png")
)
```
...
knitr::include_graphics(
  file.path(path_to_protocol, "media/Fig3Hark3.png")
)
...
knitr::include_graphics(
  file.path(path_to_protocol, "media/Fig4Secchi.png")
)
```
...
Relevante determinatiewerken worden in [Bijlage 1](#bijlage1) opgesomd.
